...
import { ThumbsUp, ThumbsDown, Meh, MessageSquare, RefreshCw, Filter, Calendar, Clock } from 'lucide-react';
...
export default function RecentItemsList({ 
  ...
  const applyFilters = useCallback(() => {
    ...
    // Always sort by newest first (since this is the recent items page)
    result.sort((a, b) => {
  ...
  const loadMoreItems = async () => {
    ...
    try {
      const response = await fetch(`/api/recent?startAfter=${items.length}`);
      
      if (!response.ok) {
        throw new Error('Failed to fetch more recent items');
      }
  ...
  if (initialItems.length === 0) {
    return (
      <div className="py-16 text-center bg-white dark:bg-gray-800 rounded-xl shadow-md p-8 border border-gray-100 dark:border-gray-700">
        ...
        <h3 className="text-2xl font-semibold text-gray-800 dark:text-white mb-3">No items found</h3>
        <p className="text-gray-600 dark:text-gray-300 mb-8 max-w-lg mx-auto">
          There are no recent items yet. Be the first to add an item to the platform!
        </p>
        <Link 
          href="/add-item" 
          className="inline-block px-6 py-3 bg-gradient-to-r from-blue-500 to-teal-500 text-white rounded-lg hover:from-blue-600 hover:to-teal-600 transition-all font-medium shadow-md"
        >
          Add New Item
        </Link>
      </div>
      ...
      <div>
        ...
        <div className="py-16 text-center bg-white dark:bg-gray-800 rounded-xl shadow-md p-8 border border-gray-100 dark:border-gray-700">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-6">
            No recent items found with your current filters.
          </p>
          <button
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md mb-6 border border-gray-100 dark:border-gray-700">
        <div className="flex items-center">
          <span className="bg-gradient-to-r from-blue-500 to-teal-500 text-transparent bg-clip-text font-medium">
      ...
      <div className="text-sm text-gray-500 dark:text-gray-400 mb-4">
        Showing {filteredItems.length} recent {filteredItems.length === 1 ? 'item' : 'items'}
            ...
            <Link href={`/item/${item.id}`} key={item.id} className="block group">
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 h-full border border-gray-100 dark:border-gray-700 flex flex-col transform hover:-translate-y-1">
                <div className="h-48 bg-gray-200 dark:bg-gray-700 relative overflow-hidden">
                  ...
                  {/* New badge */}
                  <div className="absolute top-3 left-3 bg-gradient-to-r from-blue-500 to-teal-500 text-white text-xs font-medium px-2 py-1 rounded-full flex items-center shadow-md">
      ...
      {hasMore && (
        <div className="text-center py-6">
          <button 
            ...
            disabled={isLoading}
            className="px-6 py-3 bg-gradient-to-r from-blue-500 to-teal-500 hover:from-blue-600 hover:to-teal-600 text-white rounded-lg transition-all disabled:opacity-70 disabled:cursor-not-allowed font-medium shadow-md flex items-center justify-center mx-auto transform hover:-translate-y-1 active:translate-y-0 duration-200"
          >
            {isLoading ? (
              <>
